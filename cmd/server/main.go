...
import (
	...
	"github.com/GoAsyncFunc/server-shadowsocks/internal/app/server"
	"github.com/GoAsyncFunc/server-shadowsocks/internal/pkg/service"
...
func main() {
	...
	app := &cli.App{
		...
		Version:   Version,
		Copyright: CopyRight,
		Usage:     "Provide skeleton service for the GoAsyncFunc node",
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:        "api",
			...
			// Heartbeat not strictly in skeleton flags in this version, relying on uniproxy?
			// But serviceConfig has HeartbeatInterval.
			&cli.DurationFlag{
			...
			&cli.StringFlag{
				...
				Value:       "/var/lib/server-shadowsocks",
				DefaultText: "/var/lib/server-shadowsocks",
				Required:    false,
				Destination: &dataDir,
			},
			&cli.StringFlag{
		...
		Action: func(c *cli.Context) error {
			...
			// In Skeleton, we default to Trojan for structure testing unless specified via flags (already bound).
